...
async function runValidationBenchmarks(): Promise<BenchmarkResult[]> {
    ...
    results.push(
        await benchmark(
            'sanitizeCode (dangerous)',
            () => {
                try {
                    sanitizeCode('rm -rf /');
                } catch {}
                try {
                    sanitizeCode('import os; os.system("rm -rf /")');
                } catch {}
            },
            5000
        )
    ...
    results.push(
        await benchmark(
            'validateJavaClass (invalid)',
            () => {
                try {
                    validateJavaClass('class Invalid {}');
                } catch {}
                try {
                    validateJavaClass('public class Test {} // File: Wrong.java');
                } catch {}
            },
            5000
        )
...
async function runQueueBenchmarks(): Promise<BenchmarkResult[]> {
    ...
    results.push(
        await benchmark(
            'queue.getRunningCountByLanguage',
            () => {
...
async function runOutputCollectorBenchmarks(): Promise<BenchmarkResult[]> {
    ...
    results.push(
        await benchmark(
            'OutputCollector.getFinalOutput',
            () => {
                const collector = new OutputCollector(maxBytes);
